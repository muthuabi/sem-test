...
router.post("/",async(req,res)=>{
    ...
    let role="user";
    if(email.trim()=="admin@gmail.com" || email.trim()=="muthuabi@gmail.com" || email.trim()=="sudo@gmail.com")
        role="admin"
...
router.put("/:email",async(req,res)=>{
    try{
        ...
        const user=await User.findOneAndUpdate({email:updateEmail},{name,email,password,role},{new:true});
        res.status(200).json({success:true,data:user,message:"Data Fetched",err:null});
    }
    catch(err)
    {
        res.status(400).json({success:false,message:"Error",err});
